 ...
 *     nothing. The six exhibits are a tab set — mutually exclusive by design, so
 *     they are visited one at a time rather than force-revealed together — and
 *     every output panel in them is empty until its run.
 *
 ...
 * plausible offender: it prints blinded set elements and full protocol
 * transcripts as unbroken hex.
 */
 ...
 *
 * The six exhibits are a tab set, so each is visited on its own — the state a
 * visitor is actually in. Exhibit 4's five attacks each render a separate
 * verdict, and those failure surfaces are precisely what a first-paint-only
 * gate never reaches.
 */
export async function driveAllStates(page: Page, theme: string): Promise<void> {
  await scan(page, `${theme} / first paint`);

  // Exhibit 1 — the basic intersection.
  await page.locator('#tab-1').click();
  await page.locator('#e1-run').click();
  await expect(page.locator('#e1-output')).not.toBeEmpty();
  await scan(page, `${theme} / e1 intersection`);

  // Exhibit 2 — the stepped protocol walkthrough, including a term tooltip.
  await page.locator('#tab-2').click();
  await expect(page.locator('#e2-panel')).toBeVisible();
  await scan(page, `${theme} / e2 start`);

  await page.locator('#e2-next').click();
  await expect(page.locator('#e2-step')).not.toBeEmpty();
  await scan(page, `${theme} / e2 stepped forward`);

  const term = page.locator('.e2-term').first();
  if ((await term.count()) > 0) {
    await term.hover();
    await scan(page, `${theme} / e2 term definition shown`);
  }

  // Exhibit 3 — custom sets, then the reveal.
  await page.locator('#tab-3').click();
  await page.locator('#e3-alice').fill('alice@example.com\nbob@example.com\ncarol@example.com');
  await page.locator('#e3-bob').fill('bob@example.com\ncarol@example.com\ndave@example.com');
  await page.locator('#e3-run').click();
  await expect(page.locator('#e3-output')).not.toBeEmpty();
  await scan(page, `${theme} / e3 custom sets`);

  await page.locator('#e3-reveal').click();
  await scan(page, `${theme} / e3 revealed`);

  // Exhibit 4 — all five attacks, each with its own verdict.
  await page.locator('#tab-4').click();
  for (let i = 1; i <= 5; i++) {
    await page.locator(`#e4-a${i}-run`).click();
    await expect(page.locator(`#e4-a${i}-output`)).not.toBeEmpty();
    await scan(page, `${theme} / e4 attack ${i}`);
  }

  // Exhibit 5 — the self-test suite.
  await page.locator('#tab-5').click();
  await page.locator('#e5-selftest').click();
  await scan(page, `${theme} / e5 self-test`);

  // Exhibit 6 — DDH, the benchmark and the test-vector transcript.
  await page.locator('#tab-6').click();
  await page.locator('#e6-ddh-run').click();
  await expect(page.locator('#e6-ddh-output')).not.toBeEmpty();
  await scan(page, `${theme} / e6 DDH`);

  await page.locator('#e6-bm-run').click();
  await expect(page.locator('#e6-bm-output')).not.toBeEmpty();
  await scan(page, `${theme} / e6 benchmark`);

  await page.locator('#e6-tv-run').click();
  await expect(page.locator('#e6-tv-output')).not.toBeEmpty();
  await scan(page, `${theme} / e6 test vectors`);
}
